...
export class AudioService {
    ...
    static initialize(gameNode:Node){
        ...
        this.music=host.addComponent(AudioSource);this.music.loop=true;this.music.volume=.32;
        ...
        resources.load('audio/garden_bgm',AudioClip,(error,clip)=>{if(!error&&clip){this.musicClip=clip;this.music!.clip=clip;}else console.error('背景音乐加载失败',error);done();});
    ...
    private static unlockFromGesture(){
        ...
        input.off(Input.EventType.MOUSE_DOWN,this.onFirstMouse,this);
        if(SaveService.load().music&&this.musicRequested&&this.music&&this.musicClip&&!this.music.playing)this.music.play();
    }

    static syncSettings(){
        const save=SaveService.load();
        if(save.music&&this.musicRequested)this.ensureMusic();else this.music?.stop();
    ...
    static ensureMusic(){
        ...
        if(this.gestureUnlocked&&!this.music.playing)this.music.play();
    }
